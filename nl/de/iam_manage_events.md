...
5. Entscheiden Sie, ob Sie Zugriff nach Ressourcen in einer Ressourcengruppe oder nach einzelnen Ressourcen, die in dem Konto verfügbar sind, zuweisen möchten. Sie können beispielsweise jede der folgenden Optionen wählen, um einem Benutzer eine Administratorrolle für die Verwaltung von {{site.data.keyword.at_full_notm}} zu erteilen:
...
Sie können nur 1 Instanz des {{site.data.keyword.at_full_notm}}-Service pro Position bereitstellen. Wenn Sie die Berechtigungen mit dieser Option erteilen, steuern Sie daher den Zugriff pro Position. 
{: note}
